...
private enum class Action {
    ...
    val description: String
        get() = when (this) {
            MOVE -> "Variable declaration could be moved"
    ...
    fun createFix(subjectExpressionPointer: SmartPsiElementPointer<KtExpression>): VariableDeclarationIntoWhenFix = when (this) {
        MOVE -> VariableDeclarationIntoWhenFix("Move", subjectExpressionPointer) { it }
        INLINE -> VariableDeclarationIntoWhenFix("Inline", subjectExpressionPointer) { it.initializer }
...
private class VariableDeclarationIntoWhenFix(
    ...
    override fun getName() = "$actionName variable declaration"
