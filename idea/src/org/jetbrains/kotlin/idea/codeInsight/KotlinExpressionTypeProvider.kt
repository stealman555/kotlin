...
class KotlinExpressionTypeProvider : ExpressionTypeProvider<KtExpression>() {
    ...
    private fun getTypeForArgumentName(element: KtExpression, bindingContext: BindingContext): KotlinType? {
        if (element.parent !is KtValueArgumentName) return null

        val argument = element.parent.parent as? KtValueArgument ?: return null

        val ktCallExpression = element.parents.filterIsInstance<KtCallExpression>().firstOrNull() ?: return null
